package com.book.fboard.controller;
...
public class FBoardController {
	...
	public ModelAndView board(
		...
		//게시판 목록 가져오기
		List<FBoardVo> list = fBoardService.boardList(map);
		System.out.println(list);
		//총 자료수 가져오기
		int totalCount = fBoardService.getBoardCount(map);
	...
	public ModelAndView write(@RequestParam HashMap<String, Object> map,HttpServletRequest request) {
		...
		//게시글 등록
		fBoardService.freeWrite(map,request);

		
		mv.setViewName("redirect:/FBoard/page/1");
		return mv;
	}
	...
	public ModelAndView view(@PathVariable HashMap<String, Object> map, HttpServletRequest request) {
		...
		if(userVo !=null) {
			...
			//해당 게시물의 추천 여부를 확인
			int cnt = fBoardService.likeStatus(map);
			String status;
		...
		//DB에서 게시물 정보 가져오기
		FBoardVo vo = fBoardService.boardView(map);
	...
	public int likeBoard(@RequestBody HashMap<String, Object> map) {
		...
		int cnt = fBoardService.likeBoard(map);
	...
	public ModelAndView Update(@RequestParam HashMap <String, Object> map,HttpServletRequest request) {
		...
		//DB 정보 수정
		fBoardService.update(map,request);
		
		String nowpage = (String) map.get("nowpage");
		String board_num = (String) map.get("board_num");
		
		mv.setViewName("redirect:/FBoard/View/page/"+ nowpage +"/bno/"+ board_num);
		return mv;
	}
	...
	@ResponseBody
	@RequestMapping(
			value="/ProfileFBoardList",
			method = RequestMethod.POST,
			headers = "Accept=application/json")
	public HashMap <String,Object> fboardList(@RequestBody HashMap<String, Object> map) {
	...
	public @ResponseBody void likeCancelAjax (@RequestBody HashMap <String, Object> map){
		//좋아요 취소
		fBoardService.boardLikeCancel(map);
	}
