...
<script>
  ...
  $(function(){
	  ...
	  //게시물 장르 지정
	  for (var i = 0; i <= 19 ; i++) {
		var option = $('[name=sort]')[0][i];
		if(option.value == '${ board.book_sort }'){
	  ...
	  //인자로 가져온 평점 작성
	  $('.bookGrade a').eq(${board.book_grade}).click();
...
<body>
  ...
  <form action="/BBoard/Update" method="post" enctype="multipart/form-data">
  <input type="hidden" name="nowpage" value="${ nowpage }"/>
  <input type="hidden" name="board_num" value="${ board_num }"/>
  <table class="boardContainer">
    <caption><h1>후기게시판 수정</h1></caption>
    <tr>
      <td colspan="6"><input type="text" name="title" id="title" placeholder="제목을 입력해주세요" value="${ board.board_title }"/></td>
    </tr>
    <tr>
      <th>게시자</th>
      <td colspan="5">${ board.board_writer }</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <img id="bookImg" alt="이미지 선택" src="${ board.book_image }"/></td>
      <th>책 제목</th><td colspan="4"><input type="text" name="bookTitle" id="bookTitle" placeholder="책 제목을 입력해주세요" value="${ board.book_title }"/>
      <input type="button" id="bookSearch" value="찾기"/></td>
    </tr>
    <tr>
      <th>책 저자</th><td colspan="4"><input type="text" name="author" id="author" placeholder="책 저자를 입력해주세요" value="${ board.book_author }"/></td>
    </tr>
    <tr>
      <th>출판사</th><td colspan="2"><input type="text" name="publisher" id="publisher" placeholder="출판사를 입력해주세요" value="${ board.publisher }"/></td>
      <th>출간일</th><td><input type="text" name="bookDate" id="bookDate" placeholder="출간일 입력" value="${ board.book_date }"/></td>
    </tr>
    <tr>
      <th>책 평점</th>
    ...
    <tr>
      <td colspan="6"><textarea name="cont" id="cont">${ board.board_cont }</textarea></td>
    </tr>
    <tr>
