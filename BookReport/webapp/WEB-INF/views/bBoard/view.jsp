...
<script>
  ...
  $(function(){
	    ...
	    $(document).on('click','.like',function(){
	    	...
	    	// 좋아요가 증가할 위치
	    	switch(this.id){
	    	case 'likeBoard': href = '/BBoard/LikeBoard'; 
	    	              var json = { board_num:${board.board_num},
	    ...
	    $('.delete').on('click',function(){
	    	...
	    	if(!confirm('이 게시물을 삭제하시겠습니까?'))
	    		return false;
	    	location.href = '/BBoard/Delete/'+'${ board.board_num }';
	    })
	    
	    //게시물 수정
	    $('.update').on('click',function(){
	    	location.href = '/BBoard/UpdateForm/${ nowpage }/${ board.board_num }';
	    })
...
<body>
  ...
  <table class="boardContainer">
    <caption><h1>후기게시판</h1></caption>
    <tr>
      <td colspan="6" id="title">${ board.board_title }</td>
    </tr>
    <tr>
      <th>게시자</th>
      <td colspan="5" class="left">${ board.board_writer }</td>
    </tr>
    <tr>
    ...
    <tr>
      ...
      <img id="bookImg" alt="이미지 없음" src="${ board.book_image }" /></td>
      <th>책 제목</th><td colspan="4" id="bookTitle">${ board.book_title }</td>
    </tr>
    <tr>
      <th>책 저자</th><td colspan="4" id="author">${ board.book_author }</td>
    </tr>
    <tr>
      <th>출판사</th><td colspan="2" id="publisher">${ board.publisher }</td>
      <th>출간일</th><td id="bookDate">${ board.book_date }</td>
    </tr>
    <tr>
      <th>책 평점</th>
      <td colspan="2" class="bookGrade">
      <c:forEach begin="1" end="${ board.book_grade }" step="1">
               ★
      </c:forEach>
      <c:forEach begin="1" end="${ 5 - board.book_grade }" step="1">          
               ☆
      </c:forEach>
      </td>
      <th>분류</th>
    ...
    <tr>
      <td colspan="6"><textarea id="cont" disabled>${ board.board_cont }</textarea></td>
    </tr>
    <tr>
      <td colspan="6" class="right">
      <!-- 작성자가 접속한 경우 신고는 사라지고 수정과 삭제가 등장 -->
