...
<script>
  ...
  $(function(){
		...
		if('${ board.notice_image}'=='')
			$('#imgSelect').css('display','none');
	   console.log('${ board.notice_image}');
		//좋아요 이벤트(게시물, 댓글 공통)
	    $(document).on('click','.like',function(){
	    	...
	    	// 좋아요가 증가할 위치
	    	switch(this.id){
	    	case 'likeBoard': href = '/Notice/LikeBoard'; 
	    	              var json = { notice_num:${board.notice_num},
	    ...
	    $('.delete').on('click',function(){
	    	...
	    	if(!confirm('이 게시물을 삭제하시겠습니까?'))
	    		return false;
	    	location.href = '/Notice/Delete/'+'${ board.notice_num }';
	    })
	    
	    //게시물 수정
	    $('.update').on('click',function(){
	    	location.href = '/Notice/UpdateForm/${ nowpage }/${ board.notice_num }';
	    })
	    ...
	    $(document).on('click','#submit',function(){
	    	...
	    	var data = { sort:'notice',
	    				 reply_num:reply_num,
	    			     mem_num:mem_num,
	    			     reply_cont:reply_cont,
	    				 notice_num:'${ board.notice_num }' };
	    	console.log(data);
	    ...
	    $(document).on('click','.report',e=>{
	    	...
	    	let reported_nick = $(btn).parent().siblings().children()[1].innerText;
		    let reported_mem = $(btn).parent().siblings().children()[0].value;
	    	let num = $(btn).parent().children()[0].value;
	    	let sort = '댓글';
...
<body>
  ...
  <table class="boardContainer">
    <caption><h1>공&nbsp;지&nbsp;사&nbsp;항</h1></caption>
    <tr>
      <td colspan="4" id="title">${ board.notice_title }</td>
    </tr>
    <tr>
      <th>게시자</th>
      <td>${ board.notice_writer }</td>
    ...
    <tr>
      ...
      <img id="noticeImg"  src="${ board.notice_image }" /></td>
    </tr>
    <tr>
      <td colspan="4"><textarea id="cont" disabled>${ board.notice_cont }</textarea></td>
    </tr>
    <tr>
      ...
      <c:choose>
        <c:when test="${ login.nickname eq board.notice_writer }">
      ...
      <c:if test="${ login.nickname ne board.notice_writer  && login.mem_lvl >= 100 }">
      <button class="delete">삭제</button>
      </c:if>
      </c:if>
      </td>
