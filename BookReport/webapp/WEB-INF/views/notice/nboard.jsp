...
<style>
 ...
 .searchbar input { margin-left:100px;
 					width:400px;
 					height:30px;
 					margin-right:10px; }
...
<script>
   $(function(){
	   ...
	   $('#bWrite').on('click',function(){
		   ...
		   location.href="/Notice/WriteForm";
	   })//글쓰기 끝
	   
	   //검색 버튼 클릭 시
	   $('#search').on('click',function(){
		   location.href='/Notice/page/1?keyword='+$('#keyword').val();
	   })
	   ...
	   $('.pageBtn').on('click',function(){
		    if('${keyword}'.trim() !=''){
		  	    this.href += '?keyword='+'${keyword}&sort=${sort}&sortDetail=${sortDetail}';
	   ...
	   $('th a').on('click',function(){
		   var href ='/Notice/page/1';
		   let check = false;
		   if($('#keyword').val() != ''){
		   		href += '?keyword='+$('#keyword').val();
		   		check = true;
		   }
		   if('${writer}'!=''){
			   href +='?writer=${writer}';
  ...
  <div class="searchbar">
    ...
    <table>
      ...
      <tr>
        ...
        <td><b><a href="/Notice/View/page/${ nowpage }/bno/${board.notice_num}">${ board.notice_title }</a>
        </b>&nbsp;[ ${ board.replycount } ]
        </td>
        <td><a href="/Notice/page/1?writer=${board.notice_writer}">${ board.notice_writer }</a></td>
    ...
    <c:set  var="href"  value="/Notice/page/"/>
